...
export class N8NService {
  ...
  private injectVariables(
    ...
    const replacements: Record<string, string> = {
      ...
      '{{META_PAGE_ACCESS_TOKEN}}': (config as Record<string, string>).metaPageAccessToken || '',
      '{{INSTAGRAM_USER_ID}}': (config as Record<string, string>).instagramUserId || '',
      '{{LINKEDIN_ACCESS_TOKEN}}': (config as Record<string, string>).linkedinAccessToken || '',
      '{{LINKEDIN_PERSON_URN}}': (config as Record<string, string>).linkedinPersonUrn || '',
